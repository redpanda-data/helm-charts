...
//
// +gotohelm:filename=secrets.go.tpl
package redpanda
...
func adminInternalTLSEnabled(dot *helmette.Dot) bool {
	values := helmette.Unwrap[Values](dot.Values)

	listener := values.Listeners.Admin
	enabled := ptr.Deref(listener.TLS.Enabled, values.TLS.Enabled)
	hasCert := listener.TLS.Cert != ""
	return enabled && hasCert
}

func adminTLSCurlFlags(dot *helmette.Dot) string {
	values := helmette.Unwrap[Values](dot.Values)

	if !adminInternalTLSEnabled(dot) {
		return ""
	}
