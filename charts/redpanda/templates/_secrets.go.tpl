...
{{- $snippet = (concat (default (list ) $snippet) (list `-K <(IFS=":" read -r USER PASS MECHANISM < <(grep "" $(find /etc/secrets/users/* -print)); echo "user: \"USER:PASS\"")`)) -}}
{{- end -}}
...
{{- $snippet = (concat (default (list ) $snippet) (list (printf "--cacert %s/ca.crt --cert %s/tls.crt --key %s/tls.key" $path $path $path))) -}}
{{- else -}}
{{- $snippet = (concat (default (list ) $snippet) (list (printf "--cacert %s/ca.crt" $path))) -}}
{{- end -}}
{{- end -}}
{{- $_is_returning = true -}}
{{- (dict "r" (join " " $snippet)) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
