...
{{- end -}}
{{- $_375_lockMemory_overprovisioned_flags := (get (fromJson (include "redpanda.RedpandaAdditionalStartFlags" (dict "a" (list $values) ))) "r") -}}
{{- $lockMemory := (index $_375_lockMemory_overprovisioned_flags 0) -}}
{{- $overprovisioned := (index $_375_lockMemory_overprovisioned_flags 1) -}}
{{- $flags := (index $_375_lockMemory_overprovisioned_flags 2) -}}
...
{{- $lockMemory := false -}}
{{- $_660_value_14_ok_15 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $flags "lock-memory" "") ))) "r") -}}
{{- $value_14 := (index $_660_value_14_ok_15 0) -}}
{{- $ok_15 := (index $_660_value_14_ok_15 1) -}}
{{- if $ok_15 -}}
{{- $lockMemory = (ternary (index $enabledOptions $value_14) false (hasKey $enabledOptions $value_14)) -}}
{{- $_ := (unset $flags "lock-memory") -}}
{{- end -}}
{{- $overprovisioned := false -}}
{{- $_667_value_16_ok_17 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $flags "overprovisioned" "") ))) "r") -}}
{{- $value_16 := (index $_667_value_16_ok_17 0) -}}
{{- $ok_17 := (index $_667_value_16_ok_17 1) -}}
{{- if $ok_17 -}}
...
{{- if (eq $value "") -}}
{{- $rendered = (concat (default (list ) $rendered) (list (printf "--%s" $key))) -}}
{{- else -}}
