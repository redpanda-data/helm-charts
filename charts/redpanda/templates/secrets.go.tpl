...
{{- define "redpanda.adminInternalTLSEnabled" -}}
{{- $dot := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- $values := $dot.Values.AsMap -}}
{{- $listener := $values.listeners.admin -}}
{{- $enabled := (get (fromJson (include "_shims.ptr_Deref" (dict "a" (list $listener.tls.enabled $values.tls.enabled) ))) "r") -}}
{{- $hasCert := (ne $listener.tls.cert "") -}}
{{- (dict "r" (and $enabled $hasCert)) | toJson -}}
{{- break -}}
{{- end -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $values := $dot.Values.AsMap -}}
{{- if (not (get (fromJson (include "redpanda.adminInternalTLSEnabled" (dict "a" (list $dot) ))) "r")) -}}
{{- (dict "r" "") | toJson -}}
{{- break -}}
{{- end -}}
...
{{- $dot := (index .a 0) -}}
{{- range $_ := (list 1) -}}
{{- if (get (fromJson (include "redpanda.adminInternalTLSEnabled" (dict "a" (list $dot) ))) "r") -}}
{{- (dict "r" "https") | toJson -}}
{{- break -}}
{{- end -}}
{{- (dict "r" "http") | toJson -}}
