...
{{- $s := (toJson $t) -}}
{{- $tune := (fromJson $s) -}}
{{- $_918_m_ok := (get (fromJson (include "_shims.typetest" (dict "a" (list (printf "map[%s]%s" "string" "interface {}") $tune (coalesce nil)) ))) "r") -}}
{{- $m := (index $_918_m_ok 0) -}}
{{- $ok := (index $_918_m_ok 1) -}}
{{- if (not $ok) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (dict )) | toJson -}}
{{- break -}}
...
{{- range $_, $item := $items -}}
{{- $_1035___ok_11 := (get (fromJson (include "_shims.dicttest" (dict "a" (list $seen $item.key false) ))) "r") -}}
{{- $_ := (index $_1035___ok_11 0) -}}
{{- $ok_11 := (index $_1035___ok_11 1) -}}
{{- if $ok_11 -}}
{{- continue -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_1126_cert_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $m $name (dict "enabled" (coalesce nil) "caEnabled" false "applyInternalDNSNames" (coalesce nil) "duration" "" "issuerRef" (coalesce nil) "secretRef" (coalesce nil) "clientSecretRef" (coalesce nil) )) ))) "r") -}}
{{- $cert := (index $_1126_cert_ok 0) -}}
{{- $ok := (index $_1126_cert_ok 1) -}}
{{- if (not $ok) -}}
...
{{- range $k, $v := $c -}}
{{- if (not (empty $v)) -}}
{{- $_1960___ok_18 := (get (fromJson (include "_shims.asnumeric" (dict "a" (list $v) ))) "r") -}}
{{- $_ := ((index $_1960___ok_18 0) | float64) -}}
{{- $ok_18 := (index $_1960___ok_18 1) -}}
{{- if $ok_18 -}}
{{- $_ := (set $result $k $v) -}}
...
{{- $result := (dict ) -}}
{{- range $k, $v := $c -}}
{{- $_1980_b_19_ok_20 := (get (fromJson (include "_shims.typetest" (dict "a" (list "bool" $v false) ))) "r") -}}
{{- $b_19 := (index $_1980_b_19_ok_20 0) -}}
{{- $ok_20 := (index $_1980_b_19_ok_20 1) -}}
{{- if $ok_20 -}}
{{- $_ := (set $result $k $b_19) -}}
{{- continue -}}
{{- end -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_2025___hasAccessKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_access_key" (coalesce nil)) ))) "r") -}}
{{- $_ := (index $_2025___hasAccessKey 0) -}}
{{- $hasAccessKey := (index $_2025___hasAccessKey 1) -}}
{{- $_2026___hasSecretKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_secret_key" (coalesce nil)) ))) "r") -}}
{{- $_ := (index $_2026___hasSecretKey 0) -}}
{{- $hasSecretKey := (index $_2026___hasSecretKey 1) -}}
{{- $_2027___hasSharedKey := (get (fromJson (include "_shims.dicttest" (dict "a" (list $config "cloud_storage_azure_shared_key" (coalesce nil)) ))) "r") -}}
{{- $_ := (index $_2027___hasSharedKey 0) -}}
{{- $hasSharedKey := (index $_2027___hasSharedKey 1) -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_2063___containerExists := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c "cloud_storage_azure_container" (coalesce nil)) ))) "r") -}}
{{- $_ := (index $_2063___containerExists 0) -}}
{{- $containerExists := (index $_2063___containerExists 1) -}}
{{- $_2064___accountExists := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c "cloud_storage_azure_storage_account" (coalesce nil)) ))) "r") -}}
{{- $_ := (index $_2064___accountExists 0) -}}
{{- $accountExists := (index $_2064___accountExists 1) -}}
{{- $_is_returning = true -}}
...
{{- range $_ := (list 1) -}}
{{- $_is_returning := false -}}
{{- $_2069_value_ok := (get (fromJson (include "_shims.dicttest" (dict "a" (list $c `cloud_storage_cache_size` (coalesce nil)) ))) "r") -}}
{{- $value := (index $_2069_value_ok 0) -}}
{{- $ok := (index $_2069_value_ok 1) -}}
{{- if (not $ok) -}}
{{- $_is_returning = true -}}
{{- (dict "r" (coalesce nil)) | toJson -}}
{{- break -}}
