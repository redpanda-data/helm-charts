...
func populateNamespaces(pkgs []*packages.Package) map[*types.Package]string {
	result := map[*types.Package]string{}

	for _, pkg := range pkgs {
		if _, ok := result[pkg.Types]; ok {
			continue
		}
		...
		for _, f := range pkg.Syntax {
			...
			if namespace != nil {
				panic(fmt.Sprintf("multiple namespace directives encountered in %q: %q and %q", pkg.PkgPath, ns, *namespace))
			}

			namespace = &ns
		}

		result[pkg.Types] = ptr.Deref(namespace, pkg.Name)
	}

	return result
}

// The deps slice act as all allowed go packages that can be called from particular transipled pkg
// In the deps slice the pkg can be included, but it's not mandatory
func Transpile(pkg *packages.Package, deps ...*packages.Package) (_ *Chart, err error) {
	...
	t := &Transpiler{
		...
		namespaces: populateNamespaces(append(deps, pkg)),
...
func (t *Transpiler) transpileFile(f *ast.File) *File {
	...
	// Every file name is prepended with namespace
	// To help sort autogenerated files from the other
	// the `_` character is used.
	namespace := t.namespaceFor(t.Package.Types)
	name = fmt.Sprintf("_%s.%s", namespace, name)
